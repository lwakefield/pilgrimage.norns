...
local function make_sine(n)
  return 1 * math.sin(((tau / 10) * (lfo[n].counter)) - (tau / (lfo[n].freq)))
end
...
local function make_saw(n)
  local tp = lfo[n].counter * lfo[n].freq / 10
  return 2 * (tp - math.floor(0.5 + tp))
end

local function make_tri(n)
  return 2 * math.abs(make_saw(n)) - 1
end
...
function lfo.init()
  ...
  for i = 1, number_of_outputs do
    ...
    -- lfo offset
    params:add_control(i .."offset", i .. " offset", controlspec.new(-1.0, 1.0, "lin", 0.1, 0.0, ""))
    params:set_action(i .. "offset", function(value) lfo[i].offset = value end)
    -- lfo speed
    params:add_control(i .. "lfo_freq", i .. " lfo freq", controlspec.new(0.01, 1.0, "lin", 0.01, 0.01, ""))
    ...
    for i = 1, number_of_outputs do
      if params:get(i .. "lfo") == 2 then
        ...
        elseif lfo[i].waveform == "square" then
          slope = make_square(i)
        elseif lfo[i].waveform == "saw" then
          slope = make_saw(i)
        elseif lfo[i].waveform == "tri" then
          slope = make_tri(i)
